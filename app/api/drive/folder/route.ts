...
export async function POST(request: Request) {
  try {
    const { name, starter } = await request.json();
    ...
    const result = NextResponse.json({ id: folder.id, name: folder.name, url: `https://drive.google.com/drive/folders/${folder.id}`, subfolders: names });
