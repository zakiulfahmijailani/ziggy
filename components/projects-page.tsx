...
export function ProjectsPage() {
  ...
  const createDriveFolder = async () => {
    ...
    const response = await fetch("/api/drive/folder", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, starter }) });
    const data = await response.json();
    setResult(response.ok ? data.url : data.error);
  };

  const visible = projects.filter((project) => (stage === "All" || project.stage === stage) && `${project.university} ${project.title}`.toLowerCase().includes(query.toLowerCase()));
