...
export function ProjectsPage() {
  ...
  const [folderName, setFolderName] = useState("New PhD application");
  const [starter, setStarter] = useState(true);
  const [result, setResult] = useState<string | null>(null);
  const createDriveFolder = async () => { const response = await fetch("/api/drive/folder", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: folderName, starter }) }); const data = await response.json(); setResult(response.ok ? data.url : data.error); };
  ...
  </section>{driveOpen && <div className="modal-backdrop" onClick={() => setDriveOpen(false)}><div className="modal-card" onClick={e => e.stopPropagation()}><p className="eyebrow">Drive setup</p><h2>Give Ziggy a home for your applications</h2><p className="subtle">Connect once. Ziggy will create each application folder inside your chosen Ziggy Applications folder.</p><label>New application folder name<input value={folderName} onChange={e => setFolderName(e.target.value)} /></label><label className="check-row"><input type="checkbox" checked={starter} onChange={e => setStarter(e.target.checked)} /> Create starter document folders</label><div className="modal-actions"><button className="button" onClick={() => setDriveOpen(false)}>Cancel</button><a className="button" href="/api/auth/google">Connect Google Drive</a><button className="button primary" onClick={createDriveFolder}>Create application folder</button></div>{result && (result.startsWith("http") ? <p className="success-note">Created: <a href={result} target="_blank" rel="noreferrer">Open folder</a></p> : <p className="success-note">{result}</p>)}<p className="subtle tiny">Ziggy keeps only folder IDs and links; your files remain in Google Drive.</p></div></div>}</Shell>;
}
